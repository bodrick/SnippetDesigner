...
namespace SnippetLibrary
{
    ...
    public class Utility
    {
        ...
        public static XmlElement CreateElement(XmlElement parent, string name, string innerText, XmlNamespaceManager nsMgr)
        {
            ...
            var element = parent.OwnerDocument.CreateElement(name, nsMgr.LookupNamespace("ns1"));
            var newElement = (XmlElement)parent.AppendChild(element);
        ...
        /// <param name="nsMgr">The ns MGR.</param>
        /// <returns></returns>
        public static XmlNode SetTextInDescendantElement(XmlElement element, string name, string text, XmlNamespaceManager nsMgr) => SetTextInElement(element, name, text, nsMgr, false);
